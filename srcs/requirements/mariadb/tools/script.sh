...
sleep 2
echo "create user"
mariadb -u root -p$(<"/run/secrets/db_root_password")  -e "CREATE USER IF NOT EXISTS \`${MYSQL_USER}\`@'localhost' IDENTIFIED BY '$(<"/run/secrets/db_password")';"
echo "grant privileges"
mariadb -u root -p$(<"/run/secrets/db_root_password")  -e "GRANT ALL PRIVILEGES ON \`${MYSQL_DATABASE}\`.* TO \`${MYSQL_USER}\`@'%' IDENTIFIED BY '$(<"/run/secrets/db_password")';"
